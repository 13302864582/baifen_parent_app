	...
	private void getAlbumColumnData(Cursor cur) {
		if (cur.moveToFirst()) {
			...
			do {
				...
				LogUtils.i(TAG, _id + " album:" + album + " albumArt:" + albumArt
	...
	void buildImagesBucketList() {
		...
		if (cur.moveToFirst()) {
			...
			do {
				...
				LogUtils.i(TAG, _id + ", bucketId: " + bucketId + ", picasaId: "
		...
		while (itr.hasNext()) {
			...
			LogUtils.d(TAG, entry.getKey() + ", " + bucket.getBucketName() + ", "
					+ bucket.getCount() + " ---------- ");
			for (int i = 0; i < bucket.getImageList().size(); ++i) {
				ImageItem image = bucket.getImageList().get(i);
				LogUtils.d(TAG, "----- " + image.imageId + ", " + image.imagePath
						+ ", " + image.thumbnailPath);
			}
		}
		hasBuildImagesBucketList = true;
		long endTime = System.currentTimeMillis();
		LogUtils.d(TAG, "use time: " + (endTime - startTime) + " ms");
	}
