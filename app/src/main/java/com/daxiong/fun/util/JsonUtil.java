...
public class JsonUtil {
    ...
    public static Map<String, List<String>> convetJsonObjToMap(JSONObject obj) {
        if (obj == null) {
            return null;
        }
        ...
        while (keys.hasNext()) {
            ...
            JSONArray arrays = JsonUtil.getJSONArray(obj, key, null);
            if (arrays != null) {
                for (int i = 0; i < arrays.length(); i++) {
                    try {
                        list.add(arrays.get(i).toString());
                    } catch (JSONException e) {
                    }
                }
            }
    ...
    public static JSON_TYPE getJSONType(String str){ 
        ...
        LogUtils.d("getJSONType", " firstChar = "+firstChar);          
        if(firstChar == '{'){ 
